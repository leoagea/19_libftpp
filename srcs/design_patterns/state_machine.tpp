...
/*   Updated: 2025/08/07 17:54:35 by lagea            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void StateMachine<TState>::transitionTo(const TState &state)
{
	...
	if (_currentstate == state)
		throw std::invalid_argument("Already in the requested state");

	auto range = _mm_transitions.equal_range(std::make_pair(_currentstate, state));
	if (range.first == range.second)
		throw std::invalid_argument("No transition defined for the requested state");
