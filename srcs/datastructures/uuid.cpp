...
/*   Updated: 2025/08/13 18:12:54 by lagea            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void UUID::_generateV1()
{
	...
	// Set the first 32 bits with low timestamp bits
	...
	// Set the next 16 bits with mid timestamp bits
	_data[4] = static_cast<uint8_t>((nanoseconds >> 32) & 0xFF);
	_data[5] = static_cast<uint8_t>((nanoseconds >> 40) & 0xFF);

	// Set the next 16 bits with high timestamp bits
	_data[6] = static_cast<uint8_t>(((nanoseconds >> 48) & 0x0F) | 0x10); // ✅ version 1
	_data[7] = static_cast<uint8_t>((nanoseconds >> 56) & 0xFF);          // ✅ more timestamp
	_data[8] = static_cast<uint8_t>(0x80 | (rand() & 0x3F));  
	
	// Set the remaining 8 bytes with random data
...
uint8_t UUID::hexCharToByte(char c) noexcept
{
	...
	return 0; // Should not happen if isHexDigit is used
}